...
const FlowerImage: React.FC<FlowerImageProps> = memo(({ flower, revealed }) => {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl aspect-[3/4]">
      ...
      <div
        ...
        style={{
          background:
            'linear-gradient(to top, rgba(90,40,50,0.78) 0%, rgba(90,40,50,0.12) 55%, transparent 100%)',
        }}
      />
        ...
        <AnimatePresence>
          {revealed && (
            <motion.p
              ...
              className="mt-3 text-white/75 text-sm font-serif leading-relaxed"
...
const FlowerCardItem: React.FC<{ flower: FlowerCard; index: number }> = memo(
  ({ flower, index }) => {
    ...
    // Detect pointer (hover-capable) vs touch device
    ...
    useEffect(() => {
      ...
      const handler = (e: MediaQueryListEvent) => { isPointer.current = e.matches }
      mql.addEventListener('change', handler)
      return () => mql.removeEventListener('change', handler)
    }, [])

    const handleHoverStart = useCallback(() => {
      if (isPointer.current) setRevealed(true)
    }, [])
    const handleHoverEnd = useCallback(() => {
      if (isPointer.current) setRevealed(false)
    }, [])
    const handleClick = useCallback(() => {
      if (!isPointer.current) setRevealed((p) => !p)
    }, [])
    ...
    return (
      ...
      >
        <FlowerImage flower={flower} revealed={revealed} />
      </motion.div>
    )
  }
)
...
const GallerySection: React.FC = memo(() => (
  <section
    ...
    style={{ background: 'rgba(255,240,242,0.92)', backdropFilter: 'blur(22px)' }}
  >
    <div className="max-w-4xl mx-auto">
      ...
      {/* Hint adapts to device capability */}
      <motion.p
        initial={{ opacity: 0 }}
